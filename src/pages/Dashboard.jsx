...
const Dashboard = () => {
        ...
        const fetchDashboardData = async () => {
            ...
            try {
                const fetchData = async () => {
                    // Fetch Fund Metrics with cache busting
                    const metricsRes = await fetch(`${API_URL}/dashboard/metrics?t=${Date.now()}`);
                    ...
                    const expRes = await fetch(`${API_URL}/dashboard/expenses/analytics`);
                    if (expRes.ok) {
                        const expJson = await expRes.json();
                        setExpenseData(expJson);
                    }

                    // Fetch Historical Growth
                    const histRes = await fetch(`${API_URL}/admin/growth-history`);
                    if (histRes.ok) {
    ...
    // Transform to cumulative data for "Growth" view, always call hook
    const cumulativeData = React.useMemo(() => {
        if (!expenseData?.breakdown || expenseData.breakdown.length === 0) return [];

        // Identify all keys (categories + total) from the first item (since backend ensures consistency)
        // Or better, scan all just to be safe.
        ...
        // Create a 'Start' point at 0 to ensure the chart curve starts from the bottom
    ...
    // Merge actual history from database for financial growth lines
    const growthChartData = React.useMemo(() => {
        if (!historyData || historyData.length === 0) return [];

        // Map activities to timeline points
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            {isCEO ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                    <StatCard
                        ...
                        value={metrics ? `₹${metrics.total_fund_value.toLocaleString('en-IN')}` : '₹0'}
                        icon={IndianRupee}
                    />
                    <StatCard
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-6">
                ...
                <ChartCard title="Asset Appreciation (ARR)">
                    ...
                    <div className="lg:col-span-3 mt-8">
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                            <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-4">
                                <div>
                                    <h2 className="text-xl font-bold tracking-tight text-slate-900">Expense Growth</h2>
                                ...
                                <div className="flex gap-6">
                                    <div className="text-right">
                                        <p className="text-xs font-bold text-slate-400 uppercase tracking-wider">Total Deployed</p>
                            ...
                            <div className="h-[400px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        {/* CEO View: Stacked Areas for Detail */}
                                        {isCEO ? (
                                            <>
                                                ...
                                                {/* Group remaining smaller categories for cleaner chart, or map all. Mapping all for completeness as per request. */}
                                            ...
                                            /* Investor View: Single Total Area */
